...
#else
  NSString *dataFilePath = @"/System/Library/PrivateFrameworks/AppSupport.framework/calldata.db";
#endif
  BOOL exists = [[NSFileManager defaultManager] fileExistsAtPath:dataFilePath];
  if (!exists) {
    NSString *resourcePath = [[NSBundle mainBundle] resourcePath];
    dataFilePath = [resourcePath stringByAppendingPathComponent:DATA_FILENAME];
  }
  return dataFilePath;
}
